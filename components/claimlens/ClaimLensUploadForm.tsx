...
// as multipart form-data to /api/claimlens/analyze on submit.
type Attachment = {
  id: string;
  file: File;
...
export function ClaimLensUploadForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setProgressLabel(
      attachments.length > 0
        ...
        : "Sending to ClaimLens™ for review…",
    );
    ...
    try {
      ...
      setProgressLabel("Sending to ClaimLens™ for review…");
      const res = await fetch("/api/claimlens/analyze", {
        method: "POST",
        body: form,
      });
      ...
      if (res.status === 401) {
        ...
        router.push("/login?next=/claimlens/upload");
        return;
      }
      ...
      router.push(`/claimlens/report/${data.claimId}`);
    } catch (err) {
      setSubmitting(false);
